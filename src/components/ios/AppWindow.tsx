...
export const AppWindow: React.FC<AppWindowProps> = ({ window }) => {
    ...
    const handleResizeStart = (e: React.MouseEvent) => {
        ...
        resizeStartRef.current = {
            width: typeof window.size.width === 'number' ? window.size.width : parseFloat(window.size.width),
            height: typeof window.size.height === 'number' ? window.size.height : parseFloat(window.size.height),
            x: e.clientX,
            y: e.clientY
        };
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (isDragging) {
                ...
                const maxX = viewportWidth - (typeof window.size.width === 'number' ? window.size.width : parseFloat(window.size.width)) / 2;
                const minY = 0;
