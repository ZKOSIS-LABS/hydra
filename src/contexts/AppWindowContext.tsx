...
export const AppWindowProvider: React.FC<AppWindowProviderProps> = ({ children }) => {
    ...
    const resizeApp = useCallback((id: string, size: { width: string | number; height: string | number }) => {
        setOpenWindows(prev => prev.map(window =>
    ...
    const arrangeWindows = useCallback(() => {
        ...
        if (visibleWindows.length === 1) {
            ...
            moveApp(window.id, {
                x: (viewportWidth - (typeof window.size.width === 'number' ? window.size.width : parseFloat(window.size.width))) / 2,
                y: (viewportHeight - (typeof window.size.height === 'number' ? window.size.height : parseFloat(window.size.height))) / 2
            });
